...
export class ControlsConnector {
  ...
  loop = () => {
    if (this.clock===5000) this.clock=150;
    this.sendData();
    this.timeout=setTimeout(this.loop,this.clock);
  }
  sendData = () => {
      try{
        console.log({x:this.x,y:this.y,speed:this.speed*this.max});
        this.socket.send(JSON.stringify({x:this.x,y:this.y,speed:this.speed*this.max}));
      }catch{
        if (this.socket===undefined) this.socket=socket.get();
      }
  }
